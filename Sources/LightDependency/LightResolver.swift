final class LightResolver: Resolver {
    private var disposed = false
    let resolvingContainer: LightContainer
    let resolvingStack: ResolvingStack
    
    init(resolvingContainer: LightContainer) {
        self.resolvingContainer = resolvingContainer
        resolvingStack = ResolvingStack()
    }
    
    init(resolvingContainer: LightContainer, stack: ResolvingStack) {
        self.resolvingContainer = resolvingContainer
        self.resolvingStack = stack
    }
    
    func resolve<Dependency>(file: StaticString, line: UInt) throws -> Dependency {
        guard !disposed else { resolverIsDisposedFatalError() }
        return try resolve(name: nil, file: file, line: line)
    }
    
    func resolve<Dependency>(byName name: String, file: StaticString, line: UInt) throws -> Dependency {
        guard !disposed else { resolverIsDisposedFatalError() }
        return try resolve(name: name, file: file, line: line)
    }
    
    func resolveAll<Dependency>(
